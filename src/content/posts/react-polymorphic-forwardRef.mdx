---
title: "React Polymorphic forwardRef Types that Actually Work in 2024"
pubDate: 2023-11-10
...
Credit: https://gist.github.com/kripod/4434e7cecfdecee160026aee49ea6ee8
...
export const Box = forwardRef(({ as, ...restProps }: BoxOwnProps, ref: Ref<Element>) => {
...
const Primitive = forwardRef(({ as: Comp = "div", ...props }, forwardedRef) => (
...
Credit: https://dev.to/nasheomirro/creating-fast-type-safe-polymorphic-components-3f6p
...
const Box = polyRef<"div", BoxProps>(({ as: Element = "div", ...props }, ref) => <Element ref={ref} {...props} />);
...
Or you'll get a bunch of polymorphic components that don't include forwardedRef.
Forward Ref can be a wonky but nessecary API, especially for library creators who are trying to support a wide audience with their components.

In this post, we're going to ignore non components that don't need forwardRef, setting that up is a task TypeScript is [well suited](https://www.typescriptlang.org/docs/handbook/2/generics.html#generic-constraints)
...
But we can't, that's not the interface for the function and it seems pretty tricky [to try to implement something like this](https://github.com/DefinitelyTyped/DefinitelyTyped/blob/37712a5b5c91f6f9f6f52a6e390f1f290f653a1a/types/react/index.d.ts#L822C15-L822C40).

Stricter Type Narrowing in TypeScript 5.0 made previous solutions not work as well:
...
We, as the developers, know more about the `forwardRef` function that we're composing than TypeScript can by itself, so we're overriding it with the type information we *actually* want to use.

### Why Is This So Hard?

One of the things that you'll end up trying as a React developer is to pass a `ref` as a prop in a function component, 
...
Or maybe it's because Solid returns native DOM elements instead of JavaScript? Unclear.

Regardless, being able to treat refs as if they were just any other piece of data would make this significantly easier. Maybe [React Forget](https://www.youtube.com/watch?v=lGEMwh32soc) will make these types less painful.

### Composition and `asChild`

You'll notice one of the solutions above came from a now depreciated [Radix](https://www.radix-ui.com/primitives/docs/utilities/polymorphic) package, because they're transitioned to a more compositional approach to polymorphism:
Their `asChild` method is a clever use of how React handles Props to essentially merge the props of a parent component onto the props of a child component. The result is a method that creates a compositional approach over the configuration approach from the `as` prop method:
...
The nice thing about this is that it discards the need to check a potentially changing type on one element, TypeScript will just check the types on each individual component as normal. 
And if there are conflicts between props on the parent & child, you'll usually get runtime errors for bad attributes on HTML elements, or you can create your own errors for custom components and props.
...
I like this pattern as well, and am a big fan of the compositional nature of it as well.

### Bonus Bits and Conclusion

I personally really like [Nashe Omirro](https://dev.to/nasheomirro)'s solution and accompanying [type package](https://www.npmjs.com/package/react-polymorphed) and casting `forwardRef` to an interface that handles polymorphism.
...
A nice optimization they came up with as well is to speed up `ComponentPropsWithRef` by checking all unions within `ElementType` outside of a conditional type, which sped up the type checking my a decent chunk:
...
Their package isn't on TypeScript 5.0 or higher at the time of writing, but my tests with it were positive.
